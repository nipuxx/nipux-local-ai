...
Transcription has a bundled local worker wrapper at `src/workers/transcriptionWorker.ts`. It exposes an OpenAI-compatible `POST /v1/audio/transcriptions` endpoint and shells out to a local `whisper-cli` compatible binary using `NIPUX_WHISPER_MODEL`. `bun run transcription:install` downloads a small whisper.cpp GGML model into the Nipux model directory, saves that model path in local settings, and prints both the normal local launch command and a standalone worker command. `bun run local --open` can then start the bundled transcription worker automatically when the saved model exists, while binary installation remains explicit.
