...
The Setup page is the non-dev status surface. It calls `/api/readiness`, `/api/capability-profile`, `/api/setup/actions`, and `/api/launch/profile`, then shows capability status, copyable setup commands, launch commands, clickable launcher paths, and next steps without exposing raw diagnostics by default.
...
The Agents page keeps the normal-user entry point status-first. Its Agent Setup guide summarizes the active agent and model preset, saved memory count, browser session and approval state, and local/web search readiness before exposing run, memory, browser, approval, and action-log controls. Agent runs can request local image, speech, or video generation, but those paths use the same local-only media system as the Media page and record setup failures when no matching local path is configured. Speech can use a loopback worker or the built-in local system speech fallback when available; external media APIs remain blocked. Agent run history persists tool events and media job ids so the UI can render generated media artifacts or setup failures after refresh.
...
The native responder searches indexed local documents for the latest user message, uses configured local SearXNG web search for current/web requests, executes clear local image/speech/video media requests, injects relevant context plus media tool activity into the model prompt, streams the assistant response, appends source/tool lines, and persists the assistant message with any media job ids. This keeps normal chat useful with local files, web context, and generated media while preserving the plain `/v1/chat/completions` contract for API clients.

The Chat page is the first-run status surface as well as the main conversation surface. Its Local Chat Status guide summarizes the selected model/runtime path, indexed local context and SearXNG state, local voice input/output readiness, API exposure mode, and the next copyable run/install command. When a local GGUF model is installed, it can start the managed llama.cpp runtime from the normal Chat surface without enabling dev mode; it only shows stop control for a managed runtime started by the app. The empty-chat assistant message is also derived from runtime state instead of claiming a real local model is ready. Chat-created local media artifacts render under assistant messages and survive refresh because message rows store resolved media job ids.
...
`GET /api/media/runtimes` exposes the setup plan that installers and the dev-only Media UI use. It does not install or call remote model providers; it maps each lane to a local worker contract so later bundled runtimes can be automated without changing the public app API. Configured loopback workers are health-checked and stay `offline` until a local process responds. The normal Media page surfaces compact local image, voice, and video setup guides so users can choose the recommended local image backend, see whether built-in speech is ready, copy local Whisper transcription setup commands, and understand that local video remains opt-in/backend-dependent without seeing the full backend matrix.
...
Transcription has a bundled local worker wrapper at `src/workers/transcriptionWorker.ts`. It exposes an OpenAI-compatible `POST /v1/audio/transcriptions` endpoint and shells out to a local `whisper-cli` compatible binary using `NIPUX_WHISPER_MODEL`. `GET /api/media/transcription/setup` reports the local Whisper model, worker URL, and command status. `bun run transcription:prepare --install` saves the worker URL, downloads a small whisper.cpp GGML model into the Nipux model directory, and saves that model path in local settings. `bun run local --open` can then start the bundled transcription worker automatically when the saved model exists, while binary installation remains explicit and worker health stays offline if the command is missing.
