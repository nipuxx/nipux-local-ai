...
- Media tab, media job records, and local-only image/audio/video API surfaces exist; bundled transcription worker wrapper exists, while bundled image/video runtimes are still future work.
...
- Chat microphone input records audio and can transcribe through the configured local transcription worker, including the bundled whisper.cpp-compatible worker wrapper.
- Setup page, `bun run ready`, and `bun run setup:actions` summarize readiness and expose copyable setup commands.
