...
- Main chat can create agent-owned browser sessions from clear browser/navigation requests, persist the browser session and tool events with the assistant message, approve, run, or deny pending browser navigation approvals inline, and inspect/control browser screenshots from the chat card.
