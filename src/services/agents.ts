...
function createAgentToolInstructions() {
  return [
    ...
    "Do not claim image, speech, or video media was generated unless the matching image_generation, speech_generation, or video_generation tool event is ok and a completed media job exists.",
  ].map((line) => `- ${line}`).join("\n");
}
